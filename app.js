...
const qrPlaceholderPath = "./assets/qr-placeholder.png";
...
function renderResult() {
  ...
  els.resultImage.src = state.result.image;
...
async function generateShareImage() {
  try {
    const resultImage = await loadImage(state.result.image);
    const qrImage = await loadImage(qrPlaceholderPath);
    return renderShareImage({ resultImage, qrImage });
  } catch (error) {
    console.warn("使用外部图片生成分享图失败，已切换到内置绘制。", error);
    return renderShareImage({ resultImage: null, qrImage: null });
  }
}

function renderShareImage({ resultImage, qrImage }) {
  ...
  ctx.shadowColor = "#17130f";
  ctx.shadowBlur = 0;
  ctx.shadowOffsetX = 10;
  ctx.shadowOffsetY = 10;
  ctx.strokeStyle = "#17130f";
  roundedRect(ctx, 96, 186, 708, 830, 18, false, true);
  ctx.shadowOffsetX = 0;
  ctx.shadowOffsetY = 0;
  ...
  if (resultImage) {
    ...
    drawPersonalityPlaceholder(ctx, 140, 242, 252);
  }
  ...
  ctx.fillStyle = "#17130f";
  ctx.font = "900 62px Microsoft YaHei, sans-serif";
  ...
  drawWrappedText(ctx, result.portrait.replace(/\*\*/g, ""), 136, Math.max(554, tagBottom + 34), 608, 48, 6);
  ...
  if (qrImage) {
    ...
    drawQrPlaceholder(ctx, 646, 864, 118);
  }
  ...
  try {
    ...
    if (resultImage || qrImage) {
      console.warn("Canvas 导出失败，已切换到内置绘制。", error);
      return renderShareImage({ resultImage: null, qrImage: null });
    }
    throw error;
  }
...
function drawPersonalityPlaceholder(ctx, x, y, size) {
  ...
  ctx.lineWidth = 10;
  ctx.fillStyle = "#ff5b4a";
  ctx.fillRect(60, 84, 296, 296);
  ctx.strokeRect(60, 84, 296, 296);
  ctx.fillStyle = "#ffd84f";
  ctx.beginPath();
  ctx.ellipse(375, 223, 75, 75, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.stroke();
  ctx.fillStyle = "#38d7ff";
  ctx.beginPath();
...
function drawQrPlaceholder(ctx, x, y, size) {
  ...
  pattern.forEach((row, rowIndex) => {
    Array.from(row).forEach((active, colIndex) => {
      if (active === "1") {
        ctx.fillRect(x + offset + colIndex * cell, y + offset + rowIndex * cell, cell * 0.72, cell * 0.72);
      }
    });
  });
  ctx.restore();
...
els.shareBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    els.shareImage.src = dataUrl;
    els.downloadShare.href = dataUrl;
